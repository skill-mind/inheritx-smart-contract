...
async function main() {
  ...
  const compiledClaimSierra = json.parse(
    fs.readFileSync('../target/dev/inheritx_contracts_InheritXClaim.contract_class.json').toString('ascii')
  );
  const compiledClaimCasm = json.parse(
    fs.readFileSync('../target/dev/inheritx_contracts_InheritXClaim.compiled_contract_class.json').toString('ascii')
  );

  // 4. --- Prepare Constructor Arguments ---
  const admin = process.env.ADMIN_ADDRESS;

  const myCallData = new CallData(compiledClaimSierra.abi);
  ...
  console.log('Declaring and deploying InheritXClaim contract...');
  try {
    const deployResponse = await account.declareAndDeploy({
      contract: compiledClaimSierra,
      casm: compiledClaimCasm,
    ...
    const myContract = new Contract(
      compiledClaimSierra.abi,
      deployResponse.deploy.contract_address,
      provider
    );

    console.log('✅ InheritXClaim contract connected successfully!');

    // 7. --- Write deployment summary ---
    const summary = {
      network: process.env.STARKNET_NETWORK || 'sepolia',
      account: account.address,
      contractName: 'InheritXClaim',
    ...
    const outPath = path.join(__dirname, 'claim_deployment.json');
    try {
      fs.writeFileSync(outPath, JSON.stringify(summary, null, 2));
      console.log('Saved deployment summary to', outPath);
    } catch (e) {
      console.warn('Could not write claim_deployment.json:', e);
    }

    // 8. --- Test basic functionality ---
    console.log('\n--- Testing Claim Contract ---');
    try {
